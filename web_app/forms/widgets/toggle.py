...
class ToggleWidget(forms.CheckboxInput):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.peer_focus = kwargs.pop('peer_focus', False)  # if True the toggle is focused when clicked
        self.toggle_color_on = kwargs.pop('toggle_color_on', '')  # background color when on, if empty color_on is used
        self.toggle_color_off = kwargs.pop('toggle_color_off',
                                           '')  # background color when off, if empty color_off is used
        self.toggle_color_off_soft = kwargs.pop('toggle_color_off_soft',
                                                True)  # if true the background color when off is lighter
        self.label_on = kwargs.pop('label_on',
                                   '')  # label on the right of the toggle when clicked (leave blank to not apply label)
        self.label_off = kwargs.pop('label_off',
                                    '')  # label on the right of the toggle when not clicked (leave blank to not apply label)
        ...
        self.label_wrap = kwargs.pop('label_wrap',
                                     False)  # add a tag looking wrapping to the the labels, respective color is applied
        self.label_wrap_mono = kwargs.pop('label_wrap_mono',
                                          False)  # if True the label wrap includes the toggle in a single visual element
    ...
    def get_context(self, name, value, attrs):
        ...
        context['widget'][
            'label_on_color'] = self.color_on if self.label_colored else 'gray'  # default color for labels is gray
        ...
        context['widget']['peer_focus'] = self.peer_focus
